...
test("init with explicit lab and validate passes", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "lab-os-explicit-lab-"));
...
test("init creates .ai workspace with harness namespace structure", () => {
  ...
  assert.ok(fs.existsSync(path.join(tmpDir, ".ai", ".cursor", "skills", "flow-diagram", "SKILL.md")));
