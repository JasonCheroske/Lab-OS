...
- **Canonical `LAB_ARCHITECTURE_FLOWCHART`:** use **one line per node** and **` · `** between parts. Avoid **`<br/>` / `<br>`** inside `["..."]` or `("...")`—many IDE previews print the tag literally or mangle brackets.
- Path hints: plain `lab.yaml` or `validate-lab.mjs` at end of label is fine; they **must** match real paths and the lookup table.
