...
3. **Copy or symlink the config:**
   ...
   **Option A — Copy (safest, no symlink required):**
   ```bash
   ...
   **Option B — Symlink (stays in sync automatically):**
   ...
   ```
   Note: On Windows use `mklink /D .cursor .ai\.cursor` or configure your harness to look inside `.ai/`.
...
This skill is harness-agnostic by design: it is written in plain instructions that any agent or human can follow, not in any single tool's invocation format.
